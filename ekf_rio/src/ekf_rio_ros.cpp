...
void EkfRioRos::iterate()
{
  ...
  if (queue_imu_.size() > 0)
  {
    ...
    else
    {
      ...
      if (!ekf_rio_filter_.propagate(imu_data_))
      {
  ...
  if (!queue_radar_trigger_.empty())
  {
    ...
    else
    {
      ...
      const auto time_diff_filter = ekf_rio_filter_.getTimestamp().toSec() - radar_trigger_msg.stamp.toSec();
      ...
      if (std::fabs(time_diff_filter) <= imu_data_.dt / 2)
      {
        ...
        if (radar_trigger_to_clone_delay < imu_data_.dt)
        {
          ROS_DEBUG_STREAM(kStreamingPrefix << "Adding radar clone with trigger time stamp: " << radar_trigger_msg.stamp
                                            << " at filter time " << ekf_rio_filter_.getTimestamp());
  ...
  if (!queue_radar_.empty())
  {
    ...
    if (radar_frame_id_.empty())
    {
      ...
      {
        ROS_WARN_STREAM(kStreamingPrefix << "Empty radar pcl frame id! Using default <radar>");
        radar_frame_id_ = "radar";
      }
      else
      {
    ...
    else
    {
      ...
      if (config_.run_without_radar_trigger || radar_data_msg.header.stamp.toNSec() == 0)
        radar_data_msg.header.stamp = ekf_rio_filter_.getTimestamp();
      ...
      if (ekf_rio_filter_.getTimestamp().toSec() + config_.radar_frame_ms / 1.0e3 >=
          radar_data_msg.header.stamp.toSec())
      {
        if (ekf_rio_filter_.getTimestamp().toSec() - radar_data_msg.header.stamp.toSec() > 1.0 / config_.radar_rate)
        {
        ...
        else
        {
          ...
          {
            ekf_rio_filter_.addRadarStateClone(radar_data_msg.header.stamp);
          }

          const auto time_diff_clone =
              ekf_rio_filter_.getRadarCloneState().trigger_time_stamp.toSec() - radar_data_msg.header.stamp.toSec();
          if (std::fabs(time_diff_clone) < 1.0 / config_.radar_rate)
          {
            queue_radar_.pop();

            Vector3 w_mean(0, 0, 0);
            if (radar_w_queue_.size() > 0)
            {
            ...
            if (radar_ego_velocity_.estimate(radar_data_msg, v_r, sigma_v_r, inlier_radar_scan))
            {
              ...
              bool valid = false;
              if (config_.use_w)
                valid = ekf_rio_filter_.updateRadarEgoVelocity(v_r, sigma_v_r, w_mean, config_.outlier_percentil_radar);
              else
            ...
            ROS_DEBUG_STREAM(kStreamingPrefix << "Removing radar clone with time stamp: "
                                              << ekf_rio_filter_.getRadarCloneState().time_stamp);
            ekf_rio_filter_.removeRadarStateClone();
          }
          else
          {
...
void EkfRioRos::reconfigureCallback(ekf_rio::EkfRioConfig& config, uint32_t level)
{
...
void EkfRioRos::callbackRadarTrigger(const std_msgs::HeaderConstPtr& trigger_msg)
{
  mutex_.lock();
...
void EkfRioRos::publish()
{
  ...
  if ((ekf_rio_filter_.getTimestamp() - last_timestamp_pose_pub_).toSec() > 1.0 / config_.pose_path_publisher_rate)
  {
  ...
  // covariance
  pub_cov_.publish(msg_conversion::toCovMsg(ekf_rio_filter_, config_.frame_id));

  // filter state
  pub_nom_.publish(msg_conversion::toStateMsg(ekf_rio_filter_, config_.frame_id));
  ...
  T_body_radar                 = tf2::eigenToTransform(ekf_rio_filter_.getTbr());
...
{
  const auto dataset_length  = (ekf_rio_filter_.getTimestamp() - filter_start_stamp_).toSec();
  ...
  const Vector3 p_final   = ekf_rio_filter_.getNavigationSolution().getPosition_n_b();
  const Vector3 att_final = ekf_rio_filter_.getNavigationSolution().getEuler_n_b().to_degrees();
  ...
  const Quaternion q_final_err = q_0.inverse() * ekf_rio_filter_.getNavigationSolution().getQuaternion_n_b();
